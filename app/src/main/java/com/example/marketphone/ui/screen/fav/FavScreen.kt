...
import com.example.marketphone.ui.screen.fav.FavState
...
fun FavScreen(
    ...
    viewModel.uiState.collectAsState(initial = UiState.Loading).value.let { uiState ->
        when (uiState) {
            is UiState.Loading -> {
                viewModel.getAddedOrderMarkets()
            }
            ...
            is UiState.Success -> {
                CartContent(
                    ...
                    onProductCountChanged = { rewardId, count ->
                        viewModel.updateOrderMarket(rewardId, count)
                    },
...
fun CartContent(
    ...
    Column(
        ...
        LazyColumn(
            ...
            items(state.orderMarket, key = { it.market.id }) { item ->
                FavItem(
                    marketId = item.market.id,
                    image = item.market.image,
                    title = item.market.title,
                    totalPoint = item.market.price * item.count,
                    count = item.count,
        ...
        OrderButton(
            ...
            enabled = state.orderMarket.isNotEmpty(),
            onClick = {
